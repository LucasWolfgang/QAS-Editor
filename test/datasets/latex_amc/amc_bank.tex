...
\element{cat1}{
  \begin{question}{Qsimple:img}
    We wish to pass \textit{exactly}, through $N$ given points, a \texttt{polynomial} of degree \textbf{strictly} equal to $N-1$. To find the coefficients we must solve a \emph{problem}
% Use "magic comments" to set imgResolution amc2moodle.
%amc2moodle \SetOption{imgResolution}{72}  
    \begin{center}
      \includegraphics[width=0.5\textwidth]{src/5.png}
    \end{center}
    \begin{reponses}
...
\element{cat1}{
  \begin{questionmult}{Qmult:Aucune}  \scoring{e=-0.5,b=1,m=-1.,p=-0.5} % never put b<1,
    Which fruit has a stone?
  ...
  \begin{questionmult}{Qmult:TabVerbMacro}
    ...
    \begin{center}
      \begin{tabular}{ccc}
        \hline
        12                       & 2                                                    & $2^3$ \\
        \multicolumn{2}{c}{Deux} & \includegraphics[height=16pt]{src/4.png}         \\
        \hline
      \end{tabular}
    \end{center}
    \begin{reponses}
      ...
      \mauvaise{la réponse en image \includegraphics[height=16pt]{src/4r.png}}
      \mauvaise{$6\times 6$}
...
\element{english}{
  \begin{question}{prez}
    ...
    \begin{choiceshoriz}
      ...
      \wrongchoice{with an image \includegraphics[height=16pt]{src/4r.png}}
    \end{choiceshoriz}
  \end{question}
}
...
\begin{center}
\includegraphics[height=32pt]{src/4.png}
\end{center}
...
% the associated scoring is \scoring{b=0,m=0,e=0,v=0} cf AMC doc
\end{question}
}
